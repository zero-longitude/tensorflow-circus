...
\title{Machine Learning Regression for the Econometrics Student}
...
\section{Inference vs prediction in statistics}\label{sec:cultures}
    \subsection{In econometrics we infer}
        \begin{itemize}
            ...
            \item When we do linear regression, we assume that true data generating process looks like a multivariate linear equation: \[y = X\beta + \epsilon\]
            \item Our \textbf{object of interest} is the vector of weights: $\beta$
            \item Put differently, $\beta$ is the vector of true relationships between our variables. 
        \end{itemize} 

    \subsection{Econometricians care about (a) error and (b) bias}

    \subsection{In ML we predict}
        \begin{itemize}
            \item In machine learning, our 
        \end{itemize}
